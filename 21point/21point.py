...
bot = Bot(token='5520549875:AAH105ollqZtlpe8V897ioGYFAvpIYd5Wto')
updater = Updater(token='5520549875:AAH105ollqZtlpe8V897ioGYFAvpIYd5Wto')
...
def winner_check(user, bots):
    global WINNER, WINBOTCOUNTER, WINUSERCOUNTER #ДЗ
    if sum(user) > 21 and bots < 22 or sum(user) < bots and sum(user) <= 21 and bots <= 21:
        WINNER = -1
        WINBOTCOUNTER = WINBOTCOUNTER + 1 #ДЗ
    elif bots > 21 and sum(user) < 22 or sum(user) > bots and sum(user) <= 21 and bots <= 21:
        WINNER = 1
        WINUSERCOUNTER = WINUSERCOUNTER + 1 #ДЗ
    elif sum(user) > 21 and bots > 21:
        WINNER = 0

def win_amount_check(update, context): #ДЗ
   context.bot.send_message(f"Бот выиграл {WINBOTCOUNTER} раз\n") #ДЗ
   context.bot.send_message(f"Вы выиграли {WINUSERCOUNTER} раз\n") #ДЗ


def start(update, context):
    global count_points_user, count_points_bot, WINNER, WINBOTCOUNTER, WINUSERCOUNTER #ДЗ
...
winamount_handler = CommandHandler('points', win_amount_check) #ДЗ

dispatcher.add_handler(start_handler)
dispatcher.add_handler(still_handler)
dispatcher.add_handler(stop_handler)
dispatcher.add_handler(winamount_handler) #ДЗ
